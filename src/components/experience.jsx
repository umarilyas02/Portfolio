...
function DownloadCV({ className = "" }) {
  return (
    <a
      href={CV_URL}
      download
      className={`group inline-flex items-center gap-3 rounded-full bg-ink px-7 py-3.5 text-[12px] font-semibold tracking-[0.14em] text-cream transition-[background-color,transform] duration-300 ease-out hover:-translate-y-0.5 hover:bg-pine ${className}`}
    >
      DOWNLOAD CV
      <Download
        size={15}
      ...
      <div className="mx-auto max-w-[1400px]">
        ...
        {ROLES.map((job) => (
          <FadeIn key={job.company} y={40}>
            <div className="group grid gap-6 rounded-2xl border-t border-ink/15 py-12 transition-[background-color,padding] duration-300 ease-out hover:bg-lime hover:px-5 md:grid-cols-[1fr_2fr_auto] md:gap-10 md:py-16 md:hover:px-8">
              <div>
              ...
              <span
                aria-hidden
                className="hidden h-12 w-12 shrink-0 items-center justify-center self-start rounded-full border border-ink/15 text-lg transition-all duration-300 ease-out group-hover:rotate-45 group-hover:border-ink/30 group-hover:bg-cream md:flex"
              >
                ↗
              </span>
            </div>
